...
class DetailCreativeSuccess(unittest.TestCase):
    ...
    def test_detail_creative_success(self):
        driver = self.driver
        login(self)
        self.assertIn("%s/admin/clients/" %modelConfig.baseUrl, driver.current_url, msg=None)
        sleep(1)
        ...
        self.assertIn("%s/admin/client/detail/" %modelConfig.baseUrl, driver.current_url, msg=None)

        if browser_name=="internet explorer":
            print(browser_name);
        if browser_name=="chrome" or browser_name=="firefox" or browser_name=="edge":
            posicion=driver.find_element_by_xpath('//*[@id="campaigntable"]/tbody/tr[3]/td[10]/a[1]/i')\
        ...
        self.assertIn("%s/admin/campaign/detail/" %modelConfig.baseUrl, driver.current_url,msg=None)
        posicion=driver.find_element_by_xpath('//*[@id="user-dashboard"]/div[1]/div/div[4]/a[1]')\
            .location_once_scrolled_into_view
        driver.execute_script("window.scrollTo(0, %d);" %(posicion["y"]+110))
        sleep(2)
        ...
        self.assertIn("%s/admin/creative/detail/" %modelConfig.baseUrl, driver.current_url, msg=None)
        ...
        #posicion=driver.find_element_by_xpath('//a[@href="/admin/creative/detail/%s"]' %idCreativo)\
        #    .location_once_scrolled_into_view
        #driver.execute_script("window.scrollTo(0, %d);" %(posicion["y"]+110))
        #sleep(2)
